...
public class CartDao implements CartDaoInterface {
    ...
    static List<Cart> getCartsFromResultSet(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            cart.setDiscountPercent(rs.getBigDecimal("discountPercent"));
    ...
    public boolean insertOrUpdateCart(int userId, int productId, BigDecimal discountPercent) {
        ...
        try {
            ...
            if (cart == null) {
                ...
                cart.setSubTotal(product.getPrice().subtract(product.getPrice().multiply(cart.getDiscountPercent().divide(new BigDecimal("100")))));

                result = this.createCart(cart);
            } else {
                // update + increase subTotal and quantity
                cart.setQuantity(cart.getQuantity() + 1);
                cart.setSubTotal(cart.getSubTotal().add(product.getPrice().subtract(product.getPrice().multiply(cart.getDiscountPercent().divide(new BigDecimal("100"))))));
    ...
    public boolean createCart(Cart cart) {
        ...
        try {
            ...
            String query = ""
                    + "INSERT INTO                          "
                    + "     carts (userId, productId, productImage, productName, quantity, discountPercent, subTotal) "
                    + "VALUES                               "
                    + "     (?, ?, ?, ?, ?, ?, ?)              ";
    ...
    public List<Cart> getCartByUserId(int userId) {
        ...
        return carts.size() > 0 ? carts : null;
    }
}
